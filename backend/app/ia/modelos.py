...
class PeticionInferencia(SQLModel):
    ...
    mensajes: list[MensajeChat] = Field(description="Lista de mensajes en la conversacion")
    modelo_preferido: str | None = Field(
        default=None,
        description="Identificador del modelo solicitado (opcional)"
    )
    temperatura: float = Field(
        default=0.7,
        ge=0.0,
        le=2.0,
        description="Grado de aleatoriedad en la respuesta"
    )
    max_tokens: int | None = Field(
        default=2048,
        gt=0,
        description="Tope maximo de tokens en la respuesta"
    )
...
class RespuestaInferencia(SQLModel):
    ...
    proveedor_utilizado: str = Field(description="Nombre del proveedor que respondio exitosamente")
    modelo_utilizado: str = Field(description="Identificador del modelo que genero la respuesta")
    tiempo_latencia_segundos: float = Field(description="Latencia total en segundos de la peticion exitosa")
