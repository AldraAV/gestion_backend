...
async def api_subir_storage(
    ...
    ruta_destino: str | None = Form(
        None, description="Ruta o nombre personalizado en el bucket"
    ),
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        resultado = gestor.subir_archivo(
            ruta_en_bucket=ruta_final, contenido_bytes=contenido, tipo_mime=mime
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Fallo interno en Supabase Storage: {str(e)}"
        )
...
def api_url_firmada(
    ...
    try:
        ...
        return {
            ...
            "expira_en_segundos": expiracion_segundos,
        }
    except ErrorStorageSupabase as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
