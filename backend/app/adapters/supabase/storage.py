...
class AlmacenamientoSupabase:
    ...
    def subir_archivo(
        ...
        try:
            ...
            respuesta = self._cliente.storage.from_(self.bucket).upload(
                path=ruta_en_bucket, file=contenido_bytes, file_options=opciones
            )
            return {
                "exito": True,
                "ruta": ruta_en_bucket,
            ...
            registro.error(
                "error_subir_storage_supabase",
                bucket=self.bucket,
                ruta=ruta_en_bucket,
                error=str(e),
            )
            raise ErrorStorageSupabase(
                f"Error al subir archivo a Supabase Storage: {str(e)}"
            )
    ...
    def descargar_archivo(self, ruta_en_bucket: str) -> bytes:
        ...
        try:
            ...
            registro.error(
                "error_descargar_storage_supabase",
                bucket=self.bucket,
                ruta=ruta_en_bucket,
                error=str(e),
            )
            raise ErrorStorageSupabase(
                f"Error al descargar archivo de Supabase Storage: {str(e)}"
            )
    ...
    def obtener_url_firmada(
        self, ruta_en_bucket: str, expiracion_segundos: int = 3600
    ) -> str:
        """
        Genera una URL temporal firmada para acceder a un recurso privado.
        """
        try:
            respuesta = self._cliente.storage.from_(self.bucket).create_signed_url(
                ruta_en_bucket, expiracion_segundos
            )
            return respuesta.get("signedURL", "")
        except Exception as e:
            registro.error(
                "error_generar_url_firmada", ruta=ruta_en_bucket, error=str(e)
            )
            raise ErrorStorageSupabase(f"Error al generar URL firmada: {str(e)}")

    def listar_archivos(
        self, prefijo_carpeta: str = "", limite: int = 100
    ) -> list[dict[str, Any]]:
        """
        Lista los archivos contenidos en una carpeta o prefijo del bucket.
        """
        try:
            archivos = self._cliente.storage.from_(self.bucket).list(
                path=prefijo_carpeta, options={"limit": limite}
            )
            return archivos
        except Exception as e:
