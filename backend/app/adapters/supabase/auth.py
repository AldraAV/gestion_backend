...
from .client import obtener_cliente_supabase
...
def verificar_token_supabase(token_jwt: str) -> dict[str, Any]:
    ...
    try:
        if settings.SUPABASE_JWT_SECRET:
            ...
            datos_decodificados = jwt.decode(
                token_jwt, options={"verify_signature": False, "verify_exp": True}
            )
...
def obtener_usuario_por_token(token_jwt: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not respuesta or not respuesta.user:
            raise ErrorAutenticacionSupabase(
                "No se encontro un usuario activo para el token."
            )
        ...
        raise ErrorAutenticacionSupabase(
            f"Error al validar usuario con Supabase: {str(e)}"
        )


def extraer_token_google_de_sesion(
    usuario_o_claims: dict[str, Any], cabecera_provider_token: str | None = None
) -> str | None:
    """
