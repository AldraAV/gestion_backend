...
def obtener_cliente_supabase() -> Client:
    ...
    if not settings.SUPABASE_URL or not settings.SUPABASE_KEY:
        registro.warning("supabase_credenciales_no_configuradas", url=settings.SUPABASE_URL)
        raise ErrorConfiguracionSupabase("SUPABASE_URL o SUPABASE_KEY no estan configuradas en el entorno.")
...
def obtener_cliente_supabase_admin() -> Client:
    ...
    if not settings.SUPABASE_URL or not clave_admin:
        raise ErrorConfiguracionSupabase("SUPABASE_URL o SUPABASE_SERVICE_ROLE_KEY no estan configuradas.")
