...
def descargar_archivo(
    servicio: Resource,
    archivo_id: str,
    mime_exportacion: Optional[str] = None
    ...
    try:
        ...
        if tipo_mime.startswith("application/vnd.google-apps.") and tipo_mime in MAPA_EXPORTACION_GOOGLE:
            mime_final = mime_exportacion or MAPA_EXPORTACION_GOOGLE[tipo_mime]
            peticion = servicio.files().export_media(fileId=archivo_id, mimeType=mime_final)
        ...
        registro.info("archivo_descargado_de_drive", id=archivo_id, bytes_totales=flujo_destino.tell())
        return flujo_destino.getvalue(), metadatos

    except Exception as e:
        registro.error("error_descargar_archivo_drive", id=archivo_id, error=str(e))
        raise ErrorDescargaGoogleDrive(f"Error al descargar archivo de Google Drive: {str(e)}")
