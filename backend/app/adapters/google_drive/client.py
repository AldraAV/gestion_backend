...
def obtener_servicio_drive_usuario(token_acceso_oauth: str) -> Resource:
    ...
    if not token_acceso_oauth:
        raise ErrorCredencialesGoogleDrive("Token de acceso de Google OAuth no proporcionado.")
    
    try:
        credenciales = oauth_credentials.Credentials(
            token=token_acceso_oauth,
        ...
        raise ErrorCredencialesGoogleDrive(f"Fallo al inicializar cliente de Drive con token OAuth: {str(e)}")
...
def obtener_servicio_drive_sistema() -> Resource:
    ...
    try:
        ...
        raise ErrorCredencialesGoogleDrive(f"Fallo al inicializar cliente de Drive con Service Account: {str(e)}")

def resolver_servicio_drive(token_usuario: Optional[str] = None) -> Resource:
    """
