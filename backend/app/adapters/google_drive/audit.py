    ...
    try:
        ...
        for perm in permisos:
            ...
            if tipo_perm == "anyone":
                ...
                if rol_perm in ["writer", "editor"]:
                    hallazgos.append(
                        "CRITICO: El archivo es de acceso publico editable por cualquier persona con el enlace."
                    )
                else:
                    hallazgos.append(
                        "ALTO: El archivo es de acceso publico de lectura para cualquier persona con el enlace."
                    )

            if tipo_perm == "domain":
                es_dominio_abierto = True
                hallazgos.append(
                    f"MEDIO: El archivo es accesible por todo el dominio corporativo ({perm.get('domain', 'global')})."
                )
        ...
        if es_publico:
            recomendaciones.append(
                "Restringir el permiso publico ('anyone') a correos corporativos o usuarios especificos."
            )
        if total_editores > 3:
            recomendaciones.append(
                "Revisar la lista de editores para aplicar el principio de minimo privilegio."
            )
        if not hallazgos:
            hallazgos.append(
                "Acceso controlado adecuadamente. Sin riesgos aparentes de exposicion publica."
            )
        ...
        reporte = {
            ...
            "propietarios": [
                p.get("emailAddress") for p in metadatos.get("owners", [])
            ],
        ...
        registro.info(
            "auditoria_archivo_completada", id=archivo_id, riesgo=nivel_riesgo
        )
        return reporte

    except Exception as e:
        registro.error("error_auditar_archivo_drive", id=archivo_id, error=str(e))
        raise ErrorAuditoriaGoogleDrive(
            f"Error durante la auditoria del archivo: {str(e)}"
        )


def auditar_carpeta(
    servicio: Resource, carpeta_id: str, limite_archivos: int = 50
) -> dict[str, Any]:
    """
    ...
    try:
        archivos = buscar_archivos(
            servicio, carpeta_padre_id=carpeta_id, limite=limite_archivos
        )
        reportes_archivos = []
