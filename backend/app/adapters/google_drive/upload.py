...
def crear_carpeta(
    servicio: Resource, nombre_carpeta: str, carpeta_padre_id: str | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        carpeta = (
            servicio.files()
            .create(body=metadatos, fields="id, name, mimeType, webViewLink")
            .execute()
        )

        registro.info(
            "carpeta_creada_en_drive", id=carpeta.get("id"), nombre=nombre_carpeta
        )
        return carpeta
    except Exception as e:
        registro.error("error_crear_carpeta_drive", nombre=nombre_carpeta, error=str(e))
        raise ErrorSubidaGoogleDrive(
            f"Error al crear carpeta en Google Drive: {str(e)}"
        )
...
def subir_archivo(
    ...
    try:
        ...
        medio = MediaIoBaseUpload(flujo_memoria, mimetype=tipo_mime, resumable=True)

        campos_solicitados = "id, name, mimeType, size, createdTime, modifiedTime, webViewLink, webContentLink, owners"

        archivo_creado = (
            servicio.files()
            .create(body=metadatos_cuerpo, media_body=medio, fields=campos_solicitados)
            .execute()
        )

        registro.info(
            "archivo_subido_a_drive", id=archivo_creado.get("id"), nombre=nombre_archivo
        )
